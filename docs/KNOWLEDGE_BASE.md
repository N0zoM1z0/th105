...
- Rich-header records point to compiler build 50727 and include 42 C++ LTCG
  ...
  object boundaries.
- The fresh IDA database exposes 4,001 auto-analysis function candidates. The tracked ledger contains 4,005 candidates because current target evidence recovered a missed independent CFileReader destructor at `0x0040CEB0`, source-level fighter phase entries at `0x00464630`/`0x00464780`, and standalone `CMenuReplay` scalar wrapper `0x00446F40`; IDA had attached or missed those boundaries during auto-analysis. Auto-analysis ownership and unreviewed sizes remain provisional.
- The accepted 1.06a authored set contains 549 functions / 81,536 bytes in 191
...
- Current RTTI names the shared file-list base `CFileList`; `CMenuReplay` member boundaries close the base at **0xB4**, while `ResultList` adds its character id at derived +0xB4 and is therefore 0xB8. Canonical exact ctor/dtor `0x00439F40/0x00439FC0` fix the base layout as checked `std::deque<std::string>` +0x04, `std::deque<unsigned char>` directory flags +0x18, `std::deque<unsigned>` handles +0x2C, `std::deque<FileListTile>` +0x40, three `std::string` values +0x54/+0x70/+0x8C, and tail state +0xA8/+0xAC/+0xB0. Exact `item_count/find_row/contains/trim_item_text/row_at/render_item/row_is_directory/enter_directory/format_item/shutdown` at `0x004395E0/0x00439750/0x00439810/0x00439910/0x004399A0/0x00439A20/0x00439A60/0x00439A80/0x00439B20/0x0043A0A0` independently exercise those members.
...
- `CMenuReplay` is a natural 0x280 Menu-derived lifetime: state +0x04, `CReplayList +0x08`, `TitleDesignResource +0xBC`, three UI pointers, two `MenuCursorState` values, `GuideOverlay[2]`, and checked list +0x274. Normal dtor/scalar/scene-mode virtual `0x00446D20/0x00446F40/0x00446F60` are 187/30/11 exact. The 350-byte ctor remains deliberately pending: current source and target have the same length and stores, but standalone VC8 hoists `ECX=this` before four cursor stores while target loads it immediately before the file-cursor store/call. Do not add dummy locals, volatile state, or register forcing merely to move that receiver load.
...
- Original translation-unit partition and which classes/functions underwent
  LTCG transformation.
- Accepted boundaries and authored/library origins for the remaining 2,760
