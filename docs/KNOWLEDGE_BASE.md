...
- The accepted 1.06a authored set contains 252 functions / 32,286 bytes in 123
  VC8 match units. The newest whole-corpus retained-source/lifecycle wave added 152
  functions / 18,799 bytes beyond the previous 100-function checkpoint. Candidate
...
- `PatGroup` at `0x00461520/0x00461590` is now a concrete C++ lifetime anchor, not a raw 0x20-byte view. Its first 0x10 bytes are VC8 checked `std::vector<PatRecord88>` storage; both authored ctor and dtor explicitly call `records.clear()`, and natural `/GS` source compares 106/106 and 128/128 exact. The adjacent `vector::_Tidy @ 0x0045FC30` and `vector::erase @ 0x00461320` are not authored methods: `config/vc8-generated-origin-anchors.toml` recompiles `PatGroup.cpp` with the pinned compiler, masks only COFF relocations, checks section size/coverage and full-inventory uniqueness, and classifies both as `compiler_generated`.
...
- Original translation-unit partition and which classes/functions underwent
  LTCG transformation.
- Accepted boundaries and authored/library origins for the remaining 3,077
