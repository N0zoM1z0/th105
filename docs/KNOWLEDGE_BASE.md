...
- Rich-header records point to compiler build 50727 and include 42 C++ LTCG
  ...
  object boundaries.
- The fresh IDA database exposes 4,001 auto-analysis function candidates. The tracked ledger contains 4,006 candidates because current target evidence recovered a missed independent CFileReader destructor at `0x0040CEB0`, source-level fighter phase entries at `0x00464630`/`0x00464780`, standalone `CMenuReplay` scalar wrapper `0x00446F40`, and source helper `CMenuReplay::update_mode_state @ 0x00446590`; IDA had attached or missed those boundaries during auto-analysis. The mode helper is entered by a register-restoring tail jump from `CMenuReplay::update`, then owns its own `push esi; push edi` prologue and four RET tails. Auto-analysis ownership and unreviewed sizes remain provisional.
- The accepted 1.06a authored set contains 553 functions / 83,647 bytes in 194
...
- Original translation-unit partition and which classes/functions underwent
  LTCG transformation.
- Accepted boundaries and authored/library origins for the remaining 2,757
