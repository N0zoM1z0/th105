...
- `PatRecord88_construct @ 0x00460F90` and `PatRecord88_destroy @ 0x0045FFD0` establish the current nested PAT lifetime layout: a 0x1C `FrameData` base, checked `vector<PatRaw16>` members at `+0x58/+0x68`, checked `vector<PatRaw16*>` at `+0x78`, guarded pointee/`+0x54` ownership, reverse member teardown, and base destruction. Natural `/GS` C++ is 300/300 exact for construction and 515/515 exact for destruction. The constructor store sequence matters semantically: current code clears base bytes `+0x04..+0x17` with a 0x14-byte memset, writes `tag_14 = 0`, then clears `+0x18`; widening the memset to 0x18 reorders the ownership-pointer store and fails canonical comparison. The same SHA-pinned nested-vector probe gives an inventory-unique relocation-masked fingerprint for `vector<PatRaw16>::erase @ 0x0045FBD0`, which is therefore compiler-generated rather than authored; the 42-byte vector dtor and 90-byte pointer-vector erase have current fingerprint clones and remain review-pending.
