...
- The accepted 1.06a authored set contains 608 functions / 90,161 bytes in 204
...
- Nearby stop conditions remain useful. `load_battle_setup_slot @ 0x0042BAF0` is target/fresh 332 but shares the save-slot checked-vector visibility/persistent-this blocker; exposing the vector fixes `size()` lowering but changes allocation elsewhere. `CBattleManager::transition_slot_34 @ 0x004728C0` cannot yet be accepted because phase 6 constructs `CMenuEnd` after a 0x44 allocation whose trailing four bytes remain unnamed. `pre_reset_slot_40 @ 0x00472230` similarly depends on a 0x288 `CMenuPractice` extent while +0x15C remains unresolved. `FighterPhaseContext::reset_slot_state @ 0x00465040` reaches an unknown active-slot callee `0x0064AF70`. Do not add padding, fake prototypes, register coercion, or manual vtable calls to close these.
...
- Original translation-unit partition and which classes/functions underwent
  LTCG transformation.
- Accepted boundaries and authored/library origins for the remaining 2,485
