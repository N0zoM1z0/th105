...
- The accepted 1.06a authored set contains 263 functions / 38,538 bytes in 131
  VC8 match units. The newest whole-corpus retained-source/lifecycle wave added 163
  functions / 25,051 bytes beyond the previous 100-function checkpoint. Candidate
...
- Character event bridges are now a target-backed recovery family rather than header-only declarations. Current exact members are Alice `0x0050ED00` (1075), Reimu `0x004A3220` (1061), Marisa `0x004CA980` (865), Sakuya `0x004E9B40` (843), and Patchouli `0x0052FDB0` (904). The historical event-case census is only a candidate seed: each current identity was independently checked in IDA for its character-specific case values, field writes, effect ids, action ids, virtual slots, and current address region before comparison.
...
- Original translation-unit partition and which classes/functions underwent
  LTCG transformation.
- Accepted boundaries and authored/library origins for the remaining 3,056
