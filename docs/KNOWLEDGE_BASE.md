...
- The accepted 1.06a authored set contains 616 functions / 92,178 bytes in 207
...
- Exact authored TUs are themselves a high-value compiler-origin corpus. A discovery scan across the 199 configured fresh objects found 123 standard-library COMDAT fingerprints whose complete relocation-masked hit sets contained only review candidates. Discovery results were **not** applied directly. Fifteen source-specific `config/vc8-generated-*-std-origin-anchors.toml` manifests now recompile the actual already-exact TUs under their recorded `/GS` settings and replay 206 current candidates / 24,834 bytes. The largest families are `FixedSlotEnvelope.cpp` (63 candidates / 8,358 bytes), `EffectManagerLifetime.cpp` (36 / 5,217), `CFileList.cpp` (28 / 3,452), and `BattleInputSlotConfig.cpp` (28 / 2,409). Every fingerprint still requires at least 24 exact non-relocation bytes, at least 70% non-relocation coverage, supported DIR32/REL32 forms, bounded terminal `0xCC` only where declared, and an inventory-unique hit or an exact complete equivalence group. This raises replayable compiler-generated classification from 38 to 244 without treating `std`-looking decompilation as evidence.
...
- Original translation-unit partition and which classes/functions underwent
  LTCG transformation.
- Accepted boundaries and authored/library origins for the remaining 2,477
