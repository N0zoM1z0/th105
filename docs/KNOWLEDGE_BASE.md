...
- The accepted 1.06a authored set contains 539 functions / 80,016 bytes in 189
...
- Current RTTI names the shared 0xB8 Profile/Result list base `CFileList`; retained `ProfileNameList` and `ResultListBase` are compatibility facades for that binary object. Canonical exact ctor/dtor `0x00439F40/0x00439FC0` fix the physical layout as checked `std::deque<std::string>` +0x04, `std::deque<int>` +0x18, `std::deque<unsigned>` +0x2C, `std::deque<FileListTile>` +0x40, three `std::string` values +0x54/+0x70/+0x8C, and tail state +0xA8/+0xAC/+0xB0/+0xB4. Exact `item_count/contains/trim_item_text/render_item/shutdown` at `0x004395E0/0x00439810/0x00439910/0x00439A20/0x0043A0A0` independently exercise those members.
- The same CFileList recovery closes binary `CProfileList : CFileList`: ctor `0x00431730` is 103/103 and slot-1 populate override `0x004313F0` is 29/29. The ctor uses target `profile/*.dat` through an out-of-line String28/std::string assign ABI; exposing the full inline VC8 string implementation in that caller TU expands the constructor to 240 bytes, so TU visibility is source evidence. The inherited row deque being empty triggers default-profile creation followed by a tail-call to base populate.
- `ResultList` vtable evidence corrects a retained class-contract error: slot zero is the scalar deleting destructor, not a separate `destroy()` virtual. Making the base destructor virtual and leaving the derived `ResultList` destructor implicit removes the false derived-vptr publication and yields current normal dtor `0x00447260` exactly 55/55 while preserving the existing 128/128 ctor and CMenuResult render/update/dtor exacts. Prefer the target-backed special-member shape over manual vptr stores.
- CFileList shutdown is a checked-container spelling reference. `handles_02c[i]` naturally emits the target three invalid-parameter checks and 166-byte body; `.at(i)` emits an out-of-range path and a 177-byte body. The multibyte trim helper similarly uses ordinary checked `std::string::operator[]` plus `erase`; current callers load loader-zero module-path buffer `0x00702918` as an unused receiver before byte-width helper `0x0040D1D0`. Preserve that caller-backed receiver ABI instead of flattening it to a free call.
- `CFileList::format_item @ 0x00439B20` remains intentionally pending even though the natural checked-deque-iterator/std::string source is exactly 233 bytes and matches the target control/lifetime shape. The probe emits one `std::string::assign` symbol at both copy/assignment sites, while current target uses distinct helpers `0x004020D0` and `0x00408B40`; do not map one COFF relocation symbol to two target addresses. Recover the truthful source/TU distinction first. Also treat `rank_retained_exact.py` cached `coff_size` as ranking metadata only: one stale cache reported 619 for a symbol whose saved COFF object is actually 620; acceptance always uses the actual object plus canonical comparator.
...
- Original translation-unit partition and which classes/functions underwent
  LTCG transformation.
- Accepted boundaries and authored/library origins for the remaining 2,769
