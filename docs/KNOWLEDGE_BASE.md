...
- The accepted 1.06a authored set contains 693 functions / 110,784 bytes in 222
...
- The symmetric 94-byte `acquire_and_link_object` candidates `0x00422EA0/0x00437760/0x00437BA0/0x0046F370` and 117-byte preallocators `0x00423030/0x00437840/0x00437C80/0x0046F3D0` remain review-pending. Typed template source reproduces their full semantics and exact target lengths, but standalone VC8 chooses different callee-saved registers (`this=ESI/EBX` versus target `EDI/EBP`). The shared release helpers `0x0046EEB0` (122 bytes; handle token `+0x15C`) and `0x00436E60` (119 bytes; System handle token `+0x08`) also remain pending because a direct `std::list` view emits 101 bytes and therefore misses checked-iterator ownership/validation detail. Do not force any of these with register variables, volatile lifetimes, inline assembly, or copied cleanup code.
