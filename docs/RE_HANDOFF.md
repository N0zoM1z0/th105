...
- The IDA inventory has 4,001 provisional candidates. Current reviewed state is
  61 authored functions and 3,940 still awaiting origin/boundary review.
- All 61 confirmed authored functions are source-present and canonical exact:
  5,515 exact authored bytes across 42 configured VC8 units.
- The structural-remap waves recovered fourteen functions / 2,211 bytes by using normalized
...
then expand exact recovery from the 61 accepted functions. Structural
...
The 95% authored-function and authored-byte goals cannot be reported yet: 3,940
provisional candidates still need authored/excluded classification, so the
global authored denominator is not established. Do not use the current 61/61
