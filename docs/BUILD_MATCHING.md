...
`config/vc8-generated-origin-anchors.toml` and `scripts/function-origins.py --check` make that conclusion replayable. The rule pins the target and compiler SHA-256, recompiles the tracked source with the declared `/GS` profile, requires the generated COMDAT section size to equal the candidate boundary, accepts only supported 4-byte COFF link relocations, enforces minimum exact non-relocation coverage, and requires the masked fingerprint to be unique across the full corrected inventory. Use this route for future checked-STL/template islands instead of explicit-address exclusions with prose-only evidence.
