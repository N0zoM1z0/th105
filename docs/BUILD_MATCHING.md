...
Do not use matching section length as compiler-profile proof.  `Fighter` scaled-counter helper `0x00478DF0` is target 107.  Default `/O2` and `/fp:strict` both hoist the common `scale_4D0` load and emit 99; `/fp:fast` happens to emit 107 but reassociates the x87 operations into the wrong order.  A profile that fixes size while changing semantic evaluation order is rejected.
