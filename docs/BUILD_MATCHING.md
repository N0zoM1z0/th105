...
The recovered character event bridges use sparse VC8 switches whose compiler-owned jump/index tables trail the source-level body boundary. Reimu `0x004A3220`, Marisa `0x004CA980`, Sakuya `0x004E9B40`, Patchouli `0x0052FDB0`, and Alice `0x0050ED00` all reproduce from narrow class views containing only current-target-proven Fighter fields and virtual slots. Historical same-size/event-label data is useful only to seed the current address; IDA must independently confirm the current character-specific cases before a source is accepted.
