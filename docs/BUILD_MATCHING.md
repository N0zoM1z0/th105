...
## 2026-09-05 assignment-result narrowing and giant Fighter dispatch metadata

When the target loads one full integer constant and then publishes its low word and low byte, do not assume the byte store needs a register-coercion trick.  `Udonge_reset_action_visual_state @ 0x005D06D0` is the positive reference: an int-returning assignment expression over an unsigned-byte field lets VC8 keep `255` in EAX, write AX to `+0x7A6`, write AL to `+0x113`, and return the same value.  Writing independent `opacity=255; state=-1;` statements produces an immediate byte store and misses the target.  The assignment relationship is target semantics; no volatile/register/dummy use is required.

Keep scalar-deleting wrapper evidence separate from normal-destructor exactness.  `CharacterObject` vtable-owned wrapper `0x00492F70` is 30/30 from the existing constructor TU because the normal dtor is only declared there and remains out of line.  Supplying an artificial empty dtor definition lets VC8 inline it and changes the wrapper to 34 bytes.  Conversely, a truthful nontrivial lifetime restores the out-of-line call.  Accept the wrapper when its vtable ownership and bytes are exact without pretending the separately compared normal dtor is solved.

For shared Fighter gates, repeated source predicates may be the original ownership that preserves compiler state.  `Fighter_try_dispatch_directional_action_208_210 @ 0x00478EE0` is 184/184 when three ordinary branches each retain `vertical_input_6b8 < 0`; VC8 keeps the first vertical value in EAX, retests it before later branches, and emits the shipped virtual-action tails.  Factoring the condition merely because it is redundant at the source level would erase useful target dataflow.

A giant switch's tables are compiler metadata unless control-flow review proves otherwise.  Youmu `+0x28 @ 0x00530200` ends at RET `0x0053953B`; its four switch-table islands begin at `0x0053953C` and finish before the next function at `0x005399A0`.  The three byte-indexed action regions are `0..226`, `301..546`, and `565..798`; each has a dword destination table followed by a byte index table and bounded alignment.  Compare authored bytes only through the reviewed RET, while still auditing post-RET table topology to recover source case partition/order.

Do not use matching section length as compiler-profile proof.  `Fighter_add_phase_scaled_counter_558 @ 0x00478DF0` is the positive reference.  The old flat expression made ordinary `/O2` and `/fp:strict` hoist `scale_4D0` and emit 99 bytes; `/fp:fast` happened to emit 107 but reassociated the x87 operations and was correctly rejected.  The exact solution stays on pinned ordinary `/O2` and restores source evaluation lifetime: branch-local `double x=value; x*=constant; x*=scale_4D0` emits target `fild -> constant -> scale`, while the low branch remains `scale_4D0 * value` and emits `fld/fimul`.  The result is canonical **107/107**, proving that a source-shape fix, not profile hopping, was required.

### Giant shared switch roots: separate callable code, metadata, and physical case ownership

`Fighter_update_common_action_state @ 0x004740C0` is the current reference for recovering a 10 KB shared root before the 30-80 KB roster roots that call it.  Do not compare a probe object's `.text` section size to the authored function ledger: VC8 emits the function plus local destination tables and byte-index tables in the same section.  The target callable ends at RET `0x004768AA`; compiler table data starts afterward.  The current natural probe likewise ends in code before its `$LN...` table symbols.  Compare the reviewed callable boundary, and audit table topology separately.

Use the physical switch tables as source-ownership evidence.  For this root the current target low switch has 51 destinations and the high switch has 15.  Recovering the target-proven 62/67/66 shared lane changed VC8's low table from a three-way fold at 62/63/64 into the correct three independent entries without byte padding or artificial liveness.  Conversely, attempting to preserve a destination solely by inventing locals is rejected: several safe source spellings for actions 50..52 still fold under ordinary `/O2`, and predicate rewrites that change unordered floating-point behavior are semantically invalid even when they alter code layout.

x87 operand order is source evidence, not cosmetic syntax.  The target action-145/149 gate loads 2.0 after checking the sequence and compares `2.0f > velocity_y`; writing the mathematically equivalent member-first comparison caused VC8 to hoist the constant across the switch and inject `fstp st` at unrelated case entries.  Restore the observed operand lifetime rather than compensating later with flags or fake stack variables.

Before trying `/GL`, check the actual linked prologue.  A bounded `/O2 /GL + /LTCG` experiment for `0x004740C0` produced an aligned EBP frame unlike the shipped four-byte temporary frame, so LTCG is negative provenance evidence for this root.  `/Z7`, `/Gy±`, `/Oi-` and `/Ob0` also do not change the remaining 51/52 fold; `/Oy-` changes the function frame and is rejected.  Profile experimentation is useful only to classify provenance, never to accept a size-only coincidence.

The CBitmapData lane now also demonstrates a mixed exact/nonexact unit. `/GS` source keeps ctor/scalar-dtor/dtor/copy exact and adds an exact 33-byte libpng read callback, while the two large file loaders remain deliberately nonexact on FileReaderOwner late-inline/EH scheduling. A matching section size produced by an LTCG experiment is still not acceptance: `load_from_file` can link to exactly 448 bytes while simultaneously private-specializing `FileReaderOwner::open` into the wrong ABI. Always replay the canonical target relocations and compare the actual callable bytes.

### Destructor exception/profile contracts

VC8 destructor matching can depend on two source-level contracts that must not be replaced with manual cleanup assembly. `AttackObject::~AttackObject @ 0x0045F8E0` only matches when the real `AnimationObject`/`AttackObject` destructor declarations carry `throw()`: the compiler then omits the non-target SEH frame and naturally emits the 78-byte base-cleanup chain. `CharacterObject::~CharacterObject @ 0x00492ED0` instead requires the CharacterObject TU's real `/GS` profile; changing the whole unit from `/GS-` to `/GS` preserves the exact constructor/scalar wrapper and reproduces the target 145-byte destructor, including the security cookie and EH handler. Treat exception specifications and TU flags as reconstruction evidence when sibling functions validate the same choice; never compensate with dummy locals, copied vptr writes, naked functions or assembly.

Game template virtuals need origin classification from ownership, not from the fact that VC8 instantiated a template. `BattleInfoRecordF8::bind_fighter @ 0x0046FA90` allocates `TGageValue<int*>` / `TGageValue<short*>` and writes target vtables `0x006C3890/0x006C3898`; these are source-level game classes, so their one-slot `value()` virtuals are authored. A normal template definition `return (float(*target)-start)/span` reproduces `0x0046FA50` **20/20** and `0x0046FA70` **27/27** exactly. This is distinct from `std::vector/list/map` COMDAT exclusion: generated machine-code instantiation does not imply compiler-origin source ownership. The surrounding bind method also shows a useful constructor recovery rule: target x87 arithmetic proves the stored fourth field is `end-start`, and restoring endpoint semantics brings the source to 655/648 without fake stack liveness.

CsvReader is a useful example of separating source ABI from optimizer ownership. For `next_string @ 0x0040F580`, exact callers prove an explicit destination-string parameter; target code then calls known out-of-line assignment members at `0x004021C0/0x004020D0`. Use narrow declaration-only call views to preserve those normal thiscall boundaries instead of letting the probe TU inline the full `<string>` implementation. This produces the correct 0x0C frame and 245-byte body; the remaining branch-placement residual is not permission to add a goto. For `clear_rows`, the opposite happens: natural `rows_00.clear()` is a 5-byte call/tail boundary in standalone VC8 while shipped LTCG expands `_Tidy` into 125 bytes. Exactness belongs to the separately proved 5-byte `~CsvReader() throw()` entry, not to copied STL internals. Keep native operation views and opaque lifetime facades separate when automatic member destruction would otherwise double-count a target-owned cleanup body.

### Giant-switch decompiler machine temporaries versus real source lifetimes

For giant MSVC8 switches, a Hex-Rays local that merely stores a virtual function pointer is often a machine-register artifact, not an authored C++ variable. The Youmu `+0x28` actions10/C5/C6/C7 are the positive reference: replacing explicit `vtable-slot -> function-pointer local -> call` source with ordinary direct virtual calls lets VC8 choose the shipped EAX/EDX allocation and can also prevent incorrect cross-case tail merging. This source-level correction moves the Youmu owner audit from **20/98 to 24/98** without register hints or hand-written dispatch. Do not generalize blindly: value-bearing float/facing/payload locals are retained when target stores, evaluation order, or repeated use prove a real lifetime.

A decompiler label can likewise represent false sharing. Youmu C8 and DF were both routed through `LABEL_173`, but target bytes contain separate action204 return blocks; restoring two ordinary early-return paths recovers the duplicate physical bodies. The inverse rule is equally important: moving a genuinely shared label lexically does not control VC8 block placement, and several such moves were byte-neutral. Change shared-label ownership only when target xrefs prove separate or shared source exits, never to move a branch by a desired number of bytes.

Always fresh-compile before consuming giant-owner diagnostics. Mapper/audit JSON is downstream of an object file and can look internally consistent even when a later source edit no longer compiles. The Youmu checkpoint caught exactly this after a temporary unmatched C8 brace: the old object still reported 24 owners until a direct VC8 compile exposed the syntax failure. Treat compiler provenance/mtime plus the formal unit build as a prerequisite for owner or metadata claims.
