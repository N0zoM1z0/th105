...
### CFileList/ProfileList: real STL layout, virtual lifetime, and caller visibility

Current RTTI/vtables and independent Profile/Result callers close one shared 0xB8 `CFileList` object. Natural VC8 members are checked `deque<string> +0x04`, `deque<int> +0x18`, `deque<unsigned> +0x2C`, `deque<FileListTile> +0x40`, followed by three 0x1C `std::string` values and tail state. With those members the base ctor/dtor are 128/128 and 224/224 canonical exact; hand-built 0x14/0x1C wrappers are unnecessary and lose the compiler-owned initialization/destruction schedule. Retained `ResultListBase` and `ProfileNameList` may remain narrow compatibility facades, but binary ownership is `CFileList`/`CProfileList`.

Virtual special-member shape matters. The retained ResultList facade had a nonvirtual destructor plus a fake `destroy()` slot. Current ResultList vtable instead starts with the scalar deleting destructor, then populate/finalize/shutdown/formatter. Making the base destructor virtual and leaving the derived destructor implicit naturally produces the 55-byte target `ResultList::~ResultList` and leaves its exact constructor and CMenuResult callers unchanged. Do not publish a derived vptr manually to compensate for a wrong class declaration.

Checked container APIs are also byte evidence. `CFileList::shutdown @ 0x0043A0A0` is 166/166 only with `deque<unsigned>::operator[]`; `.at()` introduces the `_Xran` path and grows to 177 bytes. `trim_item_text @ 0x00439910` is 140/140 from normal `std::string::operator[]`/`erase` and a caller-backed unused receiver at loader-zero `0x00702918` for byte-width helper `0x0040D1D0`. This is the same principle as other unused-this views: preserve the receiver a real caller supplies even when the callee body does not read ECX.

`CProfileList` demonstrates TU visibility at an STL edge. Current ctor `0x00431730` calls the shared String28/std::string assignment implementation out of line on member `+0x54`; exposing the full inline `std::string::assign` implementation in the ctor TU expands the body to 240 bytes. A narrow declaration-only assign view on the already-proved std::string subobject reproduces 103/103, while populate override `0x004313F0` is 29/29 from qualified base calls and the default-profile branch. Narrow views are appropriate when caller code proves the ABI and the underlying object layout is independently fixed.

Stop at relocation identity, not just equal length. Natural `CFileList::format_item` source with a 12-byte checked `deque<string>::iterator` compiles to the target 233-byte length and matching control flow, but fresh VC8 emits one assign symbol at two sites while the current executable reaches distinct helpers `0x004020D0` and `0x00408B40`. Mapping one source relocation to two addresses would hide an unresolved source/TU distinction, so the formatter remains pending. Likewise, cached `rank_retained_exact.py` size fields are ranking hints only: stale cache metadata can disagree with the saved COFF symbol size; read the actual object and require canonical comparison for acceptance.
