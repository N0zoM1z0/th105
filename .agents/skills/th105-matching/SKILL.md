...
3. Put the probe in the smallest plausible translation unit. Compile with `scripts/compile-unit.sh`; enable `TH105_ENABLE_GS=1` only when the target proves stack-cookie use.
4. Run `scripts/compare-function.py ADDRESS OBJECT`. Treat its relocation diagnostics as evidence, not as permission to weaken validation.
