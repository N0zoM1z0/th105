...
def render() -> str:
    ...
    authored_bytes = sum(
        owned_size(int(row["address"], 0), int(row["size"], 0), ownership)
        for row in authored
    )
...
| Confirmed authored code bytes | {authored_bytes:,} |
...
def render_svg() -> str:
    ...
    if review_pending:
        ...
        authored_filled = 0.0
        authored_detail = (
            f"{len(matches):,} exact functions · {exact_bytes:,} exact bytes · "
            f"{authored_bytes:,} confirmed authored bytes"
        )
