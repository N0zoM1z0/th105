...
def coff_symbol_bytes(
    ...
    for reloc_index in range(int(section["reloc_count"])):
        ...
        if field_offset == 0 or code[field_offset - 1] != 0xE8:
            raise ValueError(f"REL32 at +{field_offset:#x} is not an external CALL")
        ...
        target_name = coff_short_name(str(target_symbol["name"]))
        if target_name not in targets:
            raise ValueError(f"unknown external call target: {target_name}")
...
def main() -> int:
    ...
    actual_section = coff_symbol_bytes(
        obj, symbol_base, int(address, 16), size, known_targets()
    )
