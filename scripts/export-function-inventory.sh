...
ghidra_mcp_port="${TH105_GHIDRA_PORT:-8089}"

if curl --silent --fail --max-time 1 \
  "http://127.0.0.1:$ghidra_mcp_port/check_connection" >/dev/null 2>&1; then
  echo "stop the GhidraMCP server on port $ghidra_mcp_port before exporting the project inventory" >&2
  exit 1
fi
