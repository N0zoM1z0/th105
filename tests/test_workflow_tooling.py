...
class WorkflowToolingTests(unittest.TestCase):
    ...
    def test_corrected_inventory_tracks_only_current_target_progress(self) -> None:
        ...
        self.assertEqual(len(matching), 763)
        ...
        self.assertEqual(len(implemented), 763)
        self.assertEqual(
            len(self.validator.rows(ROOT / "config" / "matches.csv")), 763
        )
    ...
    def test_progress_reports_current_exact_baseline(self) -> None:
        ...
        self.assertIn("Confirmed authored functions | 763", markdown)
        self.assertIn("Classified exclusions | 931", markdown)
        self.assertIn("Origin/boundary review pending | 2,316", markdown)
        self.assertIn("Canonical exact functions | 763", markdown)
        self.assertIn("Canonical exact authored bytes | 122,625", markdown)
        self.assertIn(
