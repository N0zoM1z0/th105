...
class WorkflowToolingTests(unittest.TestCase):
    ...
    def test_corrected_inventory_tracks_only_current_target_progress(self) -> None:
        ...
        self.assertEqual(len(matching), 1084)
        ...
        self.assertEqual(len(implemented), 1084)
        self.assertEqual(
            len(self.validator.rows(ROOT / "config" / "matches.csv")), 1084
        )

    def test_match_unit_graph_covers_current_exact_baseline(self) -> None:
        manifest = self.manifest.load_manifest()
        self.assertEqual(len(manifest["units"]), 353)
        self.assertEqual(
    ...
    def test_progress_reports_current_exact_baseline(self) -> None:
        ...
        self.assertIn("Confirmed authored functions | 1,084", markdown)
        self.assertIn("Classified exclusions | 1,049", markdown)
        self.assertIn("Origin/boundary review pending | 1,877", markdown)
        self.assertIn("Canonical exact functions | 1,084", markdown)
        self.assertIn("Canonical exact authored bytes | 177,474", markdown)
        self.assertIn(
