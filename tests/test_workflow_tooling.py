...
class WorkflowToolingTests(unittest.TestCase):
    ...
    def test_corrected_inventory_tracks_only_current_target_progress(self) -> None:
        ...
        self.assertEqual(len(matching), 308)
        ...
        self.assertEqual(len(implemented), 308)
        self.assertEqual(
            len(self.validator.rows(ROOT / "config" / "matches.csv")), 308
        )
    ...
    def test_progress_reports_current_exact_baseline(self) -> None:
        ...
        self.assertIn("Confirmed authored functions | 308", markdown)
        self.assertIn("Classified exclusions | 696", markdown)
        self.assertIn("Origin/boundary review pending | 3,000", markdown)
        self.assertIn("Canonical exact functions | 308", markdown)
        self.assertIn("Canonical exact authored bytes | 51,335", markdown)
        self.assertIn(
