...
class WorkflowToolingTests(unittest.TestCase):
    ...
    def test_corrected_inventory_tracks_only_current_target_progress(self) -> None:
        ...
        self.assertEqual(len(matching), 768)
        ...
        self.assertEqual(len(implemented), 768)
        self.assertEqual(
            len(self.validator.rows(ROOT / "config" / "matches.csv")), 768
        )
    ...
    def test_progress_reports_current_exact_baseline(self) -> None:
        ...
        self.assertIn("Confirmed authored functions | 768", markdown)
        self.assertIn("Classified exclusions | 931", markdown)
        self.assertIn("Origin/boundary review pending | 2,311", markdown)
        self.assertIn("Canonical exact functions | 768", markdown)
        self.assertIn("Canonical exact authored bytes | 123,566", markdown)
        self.assertIn(
