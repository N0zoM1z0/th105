...
class WorkflowToolingTests(unittest.TestCase):
    ...
    def test_corrected_inventory_tracks_only_current_target_progress(self) -> None:
        ...
        self.assertEqual(len(matching), 1122)
        ...
        self.assertEqual(len(implemented), 1122)
        self.assertEqual(
            len(self.validator.rows(ROOT / "config" / "matches.csv")), 1122
        )

    def test_match_unit_graph_covers_current_exact_baseline(self) -> None:
        manifest = self.manifest.load_manifest()
        self.assertEqual(len(manifest["units"]), 376)
        self.assertEqual(
    ...
    def test_progress_reports_current_exact_baseline(self) -> None:
        ...
        self.assertIn("Confirmed authored functions | 1,122", markdown)
        self.assertIn("Classified exclusions | 1,049", markdown)
        self.assertIn("Origin/boundary review pending | 1,839", markdown)
        self.assertIn("Canonical exact functions | 1,122", markdown)
        self.assertIn("Canonical exact authored bytes | 186,782", markdown)
        self.assertIn(
