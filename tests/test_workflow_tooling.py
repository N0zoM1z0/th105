...
class WorkflowToolingTests(unittest.TestCase):
    ...
    def test_corrected_inventory_tracks_only_current_target_progress(self) -> None:
        ...
        self.assertEqual(len(matching), 309)
        ...
        self.assertEqual(len(implemented), 309)
        self.assertEqual(
            len(self.validator.rows(ROOT / "config" / "matches.csv")), 309
        )

    def test_match_unit_graph_covers_current_exact_baseline(self) -> None:
        manifest = self.manifest.load_manifest()
        self.assertEqual(len(manifest["units"]), 154)
        self.assertEqual(
    ...
    def test_progress_reports_current_exact_baseline(self) -> None:
        ...
        self.assertIn("Confirmed authored functions | 309", markdown)
        self.assertIn("Classified exclusions | 696", markdown)
        self.assertIn("Origin/boundary review pending | 2,999", markdown)
        self.assertIn("Canonical exact functions | 309", markdown)
        self.assertIn("Canonical exact authored bytes | 51,524", markdown)
        self.assertIn(
