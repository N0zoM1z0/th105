...
class WorkflowToolingTests(unittest.TestCase):
    ...
    def test_corrected_inventory_tracks_only_current_target_progress(self) -> None:
        ...
        self.assertEqual(len(matching), 56)
        ...
        self.assertEqual(len(implemented), 56)
        self.assertEqual(
            len(self.validator.rows(ROOT / "config" / "matches.csv")), 56
        )

    def test_match_unit_graph_covers_current_exact_baseline(self) -> None:
        manifest = self.manifest.load_manifest()
        self.assertEqual(len(manifest["units"]), 37)
        self.assertEqual(
    ...
    def test_progress_reports_current_exact_baseline(self) -> None:
        ...
        self.assertIn("Confirmed authored functions | 56", markdown)
        self.assertIn("Canonical exact functions | 56", markdown)
        self.assertIn("Canonical exact authored bytes | 4,644", markdown)
        self.assertIn(
