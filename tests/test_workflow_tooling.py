...
class WorkflowToolingTests(unittest.TestCase):
    ...
    def test_corrected_inventory_tracks_only_current_target_progress(self) -> None:
        ...
        self.assertEqual(len(matching), 661)
        ...
        self.assertEqual(len(implemented), 661)
        self.assertEqual(
            len(self.validator.rows(ROOT / "config" / "matches.csv")), 661
        )

    def test_match_unit_graph_covers_current_exact_baseline(self) -> None:
        manifest = self.manifest.load_manifest()
        self.assertEqual(len(manifest["units"]), 209)
        self.assertEqual(
    ...
    def test_progress_reports_current_exact_baseline(self) -> None:
        ...
        self.assertIn("Confirmed authored functions | 661", markdown)
        self.assertIn("Classified exclusions | 927", markdown)
        self.assertIn("Origin/boundary review pending | 2,422", markdown)
        self.assertIn("Canonical exact functions | 661", markdown)
        self.assertIn("Canonical exact authored bytes | 106,503", markdown)
        self.assertIn(
