...
class WorkflowToolingTests(unittest.TestCase):
    ...
    def test_corrected_inventory_tracks_only_current_target_progress(self) -> None:
        ...
        self.assertEqual(len(matching), 284)
        ...
        self.assertEqual(len(implemented), 284)
        self.assertEqual(
            len(self.validator.rows(ROOT / "config" / "matches.csv")), 284
        )
    ...
    def test_progress_reports_current_exact_baseline(self) -> None:
        ...
        self.assertIn("Confirmed authored functions | 284", markdown)
        self.assertIn("Classified exclusions | 696", markdown)
        self.assertIn("Origin/boundary review pending | 3,024", markdown)
        self.assertIn("Canonical exact functions | 284", markdown)
        self.assertIn("Canonical exact authored bytes | 49,640", markdown)
        self.assertIn(
