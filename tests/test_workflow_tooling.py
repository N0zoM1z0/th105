...
class WorkflowToolingTests(unittest.TestCase):
    ...
    def test_corrected_inventory_tracks_only_current_target_progress(self) -> None:
        ...
        self.assertEqual(len(matching), 873)
        ...
        self.assertEqual(len(implemented), 873)
        self.assertEqual(
            len(self.validator.rows(ROOT / "config" / "matches.csv")), 873
        )

    def test_match_unit_graph_covers_current_exact_baseline(self) -> None:
        manifest = self.manifest.load_manifest()
        self.assertEqual(len(manifest["units"]), 267)
        self.assertEqual(
    ...
    def test_progress_reports_current_exact_baseline(self) -> None:
        ...
        self.assertIn("Confirmed authored functions | 873", markdown)
        self.assertIn("Classified exclusions | 1,009", markdown)
        self.assertIn("Origin/boundary review pending | 2,128", markdown)
        self.assertIn("Canonical exact functions | 873", markdown)
        self.assertIn("Canonical exact authored bytes | 140,219", markdown)
        self.assertIn(
