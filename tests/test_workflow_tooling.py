...
class WorkflowToolingTests(unittest.TestCase):
    ...
    def test_corrected_inventory_tracks_only_current_target_progress(self) -> None:
        ...
        self.assertEqual(len(matching), 68)
        ...
        self.assertEqual(len(implemented), 68)
        self.assertEqual(
            len(self.validator.rows(ROOT / "config" / "matches.csv")), 68
        )
    ...
    def test_progress_reports_current_exact_baseline(self) -> None:
        ...
        self.assertIn("Confirmed authored functions | 68", markdown)
        self.assertIn("Classified exclusions | 480", markdown)
        self.assertIn("Origin/boundary review pending | 3,453", markdown)
        self.assertIn("Canonical exact functions | 68", markdown)
        self.assertIn("Canonical exact authored bytes | 7,599", markdown)
        self.assertIn(
