...
// Narrow 1.06a source view for Youmu's event bridge. Only fields, access widths,
...
class YoumuEventView {
public:
    virtual ~YoumuEventView();
...
public:
    ...
    YoumuEventView *other_fighter_170;
...
typedef char YoumuEventView_state_13c_offset[
    offsetof(YoumuEventView, state_13c) == 0x13c ? 1 : -1];
typedef char YoumuEventView_other_offset[
    offsetof(YoumuEventView, other_fighter_170) == 0x170 ? 1 : -1];
typedef char YoumuEventView_event_counter_offset[
    offsetof(YoumuEventView, event_counter_730) == 0x730 ? 1 : -1];
typedef char YoumuEventView_event_flag_offset[
    offsetof(YoumuEventView, event_flag_759) == 0x759 ? 1 : -1];

unsigned char YoumuEventView::handle_event_bridge(int event_code)
{
    switch (event_code) {
    case 1:
        x_ec = 100.0f;
    ...
    case 103:
        event_subobject_130.trigger_global_effect(3);
        event_flag_759 = 1;
        event_value_75c = 0;
        event_value_760 = 180;
        return false;
    ...
    case 105: {
        YoumuEventView *const other = other_fighter_170;
        y_f0 = 960.0f;
