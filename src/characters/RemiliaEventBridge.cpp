...
// Narrow 1.06a source view for Remilia's event bridge. Only fields, access widths,
...
class RemiliaEventView {
public:
    virtual ~RemiliaEventView();
...
public:
    ...
    RemiliaEventView *other_fighter_170;
...
typedef char RemiliaEventView_state_13c_offset[
    offsetof(RemiliaEventView, state_13c) == 0x13c ? 1 : -1];
typedef char RemiliaEventView_other_offset[
    offsetof(RemiliaEventView, other_fighter_170) == 0x170 ? 1 : -1];
typedef char RemiliaEventView_event_counter_offset[
    offsetof(RemiliaEventView, event_counter_730) == 0x730 ? 1 : -1];
typedef char RemiliaEventView_event_flag_offset[
    offsetof(RemiliaEventView, event_flag_759) == 0x759 ? 1 : -1];

unsigned char RemiliaEventView::handle_event_bridge(int event_code)
{
    switch (event_code) {
    case 1:
    ...
    case 103:
        event_subobject_130.trigger_global_effect(6);
        event_flag_759 = 1;
        event_value_75c = 0;
        event_value_760 = 180;
        return false;
    ...
    case 105: {
        RemiliaEventView *const other = other_fighter_170;
        y_f0 = 960.0f;
        x_ec = other->x_ec +
