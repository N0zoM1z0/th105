...
// Narrow 1.06a source view for Sakuya's event bridge. The layout contains only
// fields and virtual slots directly established by current target control flow.
...
class SakuyaEventView {
public:
    virtual ~SakuyaEventView();
    ...
    SakuyaEventView *other_fighter_170;
...
typedef char SakuyaEvent_state_offset[
    offsetof(SakuyaEventView, state_13c) == 0x13c ? 1 : -1];
typedef char SakuyaEvent_other_offset[
    offsetof(SakuyaEventView, other_fighter_170) == 0x170 ? 1 : -1];
typedef char SakuyaEvent_counter_offset[
    offsetof(SakuyaEventView, event_counter_730) == 0x730 ? 1 : -1];
typedef char SakuyaEvent_flag_offset[
    offsetof(SakuyaEventView, event_flag_759) == 0x759 ? 1 : -1];

unsigned char SakuyaEventView::handle_event_bridge(int event_code)
{
    switch (event_code) {
    case 1:
    ...
    case 103:
        event_subobject_130.trigger_global_effect(2);
        event_flag_759 = 1;
        event_value_75c = 0;
        event_value_760 = 180;
        return 0;
    ...
    case 105: {
        SakuyaEventView *const other = other_fighter_170;
        y_f0 = 960.0f;
        x_ec = other->x_ec +
