...
// Narrow current-target view for the Alice event bridge. Only fields and virtual
// slots proven by 1.06a IDA control flow are represented here; the complete
// derived Alice layout remains intentionally unresolved.
...
class AliceEventView {
public:
    virtual ~AliceEventView();
...
public:
    ...
    AliceEventView *other_fighter_170;
...
typedef char AliceEventView_state_13c_offset[
    offsetof(AliceEventView, state_13c) == 0x13c ? 1 : -1];
typedef char AliceEventView_other_offset[
    offsetof(AliceEventView, other_fighter_170) == 0x170 ? 1 : -1];
typedef char AliceEventView_event_counter_offset[
    offsetof(AliceEventView, event_counter_730) == 0x730 ? 1 : -1];
typedef char AliceEventView_event_flag_offset[
    offsetof(AliceEventView, event_flag_759) == 0x759 ? 1 : -1];

unsigned char AliceEventView::handle_event_bridge(int event_code)
{
    switch (event_code) {
    case 1:
        x_ec = 100.0f;
    ...
    case 103:
        event_subobject_130.trigger_global_effect(4);
        event_flag_759 = 1;
        event_value_75c = 0;
        event_value_760 = 180;
        return false;
    ...
    case 105: {
        AliceEventView *const other = other_fighter_170;
        y_f0 = 960.0f;
        x_ec = other->x_ec +
